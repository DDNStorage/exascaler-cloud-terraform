...
resource "azurerm_network_interface" "cls" {
  ...
  name                          = format("%s-%s%d-%s", local.prefix, "cls", count.index, "network-interfrace")
  location                      = local.resource_group.location
  resource_group_name           = local.resource_group.name
...
resource "azurerm_network_interface_application_security_group_association" "cls" {
  count                         = var.security.enable_ssh && var.cls.public_ip ? var.cls.node_count : 0
...
resource "azurerm_role_assignment" "cls" {
  count                = var.cls.node_count
  scope                = local.resource_group.id
  principal_id         = azurerm_linux_virtual_machine.cls[count.index].identity.0.principal_id
