...
resource "azurerm_network_interface" "oss" {
  ...
  name                          = format("%s-%s%d-%s", local.prefix, "oss", count.index, "network-interfrace")
  location                      = local.resource_group.location
  resource_group_name           = local.resource_group.name
...
resource "azurerm_network_interface_application_security_group_association" "oss" {
  count                         = var.security.enable_ssh && var.oss.public_ip ? var.oss.node_count : 0
...
resource "azurerm_role_assignment" "oss" {
  count                = var.oss.node_count
  scope                = local.resource_group.id
  principal_id         = azurerm_linux_virtual_machine.oss[count.index].identity.0.principal_id
